...
type AudienceBody = {
  step: 0 | 1 | 2 | 3 | "final_edit" | "refine_existing";
  input?: string;
  state?: {
    who?: string;
    needs?: string;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (step === "refine_existing") {
      ...
      const finalMsg =
        `Jag har förbättrat beskrivningen enligt det du skrev:\n\n${profile}\n\n` +
        `Vill du ändra något mer, eller ska vi spara den här versionen?`;

      return res.status(200).json({
        ok: true,
        ui: [{ role: "assistant", id: "audience_refined", text: finalMsg }],
        data: { audience_candidate: profile },
        actions: [{ type: "offer_edit_or_save", field: "audience_profile" }],
        next_step: "done",
      });
